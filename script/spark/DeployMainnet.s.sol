...
contract DeployScript is Script {
    function run() public returns (AaveV3ERC4626Factory deployed) {
        ...
        address rewardsController = vm.envAddress("SPARK_REWARDS_CONTROLLER_MAINNET");
        address rewardRecipient = vm.envAddress("SPARK_REWARDS_RECIPIENT_MAINNET");
        address lendingPool = vm.envAddress("SPARK_LENDING_POOL_MAINNET");

        vm.startBroadcast(deployerPrivateKey);

        deployed = AaveV3ERC4626Factory(
            create3.deploy(
                keccak256("AaveV3ERC4626Factory-Spark"),
                bytes.concat(
